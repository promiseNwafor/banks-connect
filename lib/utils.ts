...
export function validateStateAbbreviation(state: string): string | null {
  ...
  return stateAbbreviations.includes(formattedState) ? formattedState : null
}

export function formatSSN(ssn: string): string | null {
  // Remove any non-digit characters
  const cleanSSN = ssn.replace(/\D/g, '')

  // Check if the cleaned SSN is exactly 9 digits
